...
def SimpleLSTM(conf_path, input_tensor, seq_length):
    ...
    with tf.name_scope("lstm"):
        ...
        def cell():
            return tf.contrib.rnn.BasicLSTMCell(n_hidden_units, state_is_tuple=True)
...
def BiRNN(conf_path, batch_x, seq_length, n_input, n_context):
    ...
    with tf.name_scope("fc3"):
        b3 = variable_on_cpu(
            "b3", [n_hidden_3], tf.random_normal_initializer(stddev=b3_stddev)
        )
        h3 = variable_on_cpu(
            "h3",
    ...
    with tf.name_scope("fc6"):
        ...
        b6 = variable_on_cpu(
            "b6", [n_hidden_6], tf.random_normal_initializer(stddev=b6_stddev)
        )
        h6 = variable_on_cpu(
            "h6",
