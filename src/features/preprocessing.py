...
Liwicki, M. ; Bunke, H.:  HMM-based on-line recognition  of  handwritten  whiteboard
notes. In: Tenth  International Workshop on Frontiers in Handwriting Recognition,
Suvisoft, 2006
...
def rot_x(x, y, min_x, min_y, cos_alpha, sin_alpha):
    return min_x + cos_alpha * (x - min_x) - sin_alpha * (y - min_y)


def rot_y(x, y, min_x, min_y, cos_alpha, sin_alpha):
    return min_y + sin_alpha * (x - min_x) + cos_alpha * (y - min_y)
...
def correct_slope(ink):
    ...
    new_ink = np.array(
        [
            [
                rot_x(x, y, min_x, min_y, cos_alpha, sin_alpha),
                rot_y(x, y, min_x, min_y, cos_alpha, sin_alpha),
                p,
            ]
            for [x, y, p] in ink
        ]
...
def correct_slant(ink):
    ...
    for cur_point in ink[1:]:
        ...
        if (cur_point[0] - last_point[0]) == 0:
            ...
            angle = (
                math.atan(
                    (cur_point[1] - last_point[1]) / float(cur_point[0] - last_point[0])
                )
                * 180
                / math.pi
            )
    ...
    smoothed = (
        [bins[0]]
        + [gauss(bins[i - 1], bins[i], bins[i + 1]) for i in range(len(bins) - 1)]
        + [bins[len(bins) - 1]]
    )
    ...
    min_y = min(ink[:, 1])
    rotate = (
        lambda x, y: min_x + (x - min_x) - math.tan(slant * math.pi / 180) * (y - min_y)
    )
    ...
    while i < len_ink:
        ...
        while length <= current_length and i < len_ink:
            ...
            if i < len_ink:
                ...
                curr = i
                old_length = length
                length += math.sqrt((ink[curr, 0] - ink[last, 0]) ** 2) + math.sqrt(
                    (ink[curr, 1] - ink[last, 1]) ** 2
                )
        if i < len_ink:
...
def smoothing(ink):
    ...
    smoothed = np.array(
        [gauss(ink[i - 1], ink[i], ink[i + 1]) for i in range(1, ink.shape[0] - 1)]
    )
